...
async def test_single_build():
    ...
    try:
        ...
        logger.info(f"📤 Uploading generated image...")
        ...
        if not url:
            logger.error(f"❌ Failed to upload character image")
            return False
        ...
        if result.data:
            logger.info(f"✅ Stored test build in database")
        else:
            logger.error(f"❌ Failed to store test build in database")
            
        # Verify preset portraits are still there
        logger.info("🔍 Verifying preset portraits are still accessible...")
        for i, portrait in enumerate(PRESET_PORTRAITS["male"][:2]):  # Check first 2
